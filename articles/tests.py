...
class ArticleCreateTest(APITestCase):
    ...
    def setUpTestData(cls):
        cls.user_data = {'email':'hyun@naver.com', 'password':'1234'}
        cls.article_data = {'title':'some title', 'content': 'some content'}
        cls.user = User.objects.create_user('hyun@naver.com', '1234')
        # cls.article = Article.objects.create_article('some title', 'some content')
...
class ArticleReadTest(APITestCase):
    @classmethod
    def setUpTestData(cls): # test 데이터 만들기
        cls.faker = Faker()
        cls.articles= []
        for i in range(10):
